...
import { DoctorApiApi, DoctorDTO, DoctorFilterDTO } from "@generated/axios";
...
const Doctors: React.FC = () => {
  ...
  const doctorContextValue = useMemo(() => ({ filter, dispatch }), [filter, dispatch]);
  const navigate = useNavigate();

  const [doctors, loading] = useGetList<DoctorDTO, DoctorFilterDTO>(api.getListDoctor.bind(api), filter);

  return (
    <DoctorsFilterContext.Provider value={doctorContextValue}>
      <Breadcrumb>
